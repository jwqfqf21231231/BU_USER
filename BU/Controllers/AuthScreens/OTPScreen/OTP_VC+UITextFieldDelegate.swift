//
//  OTP_VC+UITextFieldDelegate.swift
//  BU
//
...
extension OTP_VC:UITextFieldDelegate{
    ...
    func textFieldShouldReturn(_ textField: UITextField) -> Bool {
        guard let currentIndexPath = customMethodManager?.getIndexPathFor(view: textField, tableView: self.otpTable) else { return true }
        let lastRowIndex = self.otpTable.numberOfRows(inSection: 0) - 1
        if currentIndexPath.row != lastRowIndex {
            ...
            while nextIndexPath.row <= lastRowIndex {
                if let nextCell = self.otpTable.cellForRow(at: nextIndexPath) as? Auth_TextField_TableViewCell {
                    self.otpTable.scrollToRow(at: nextIndexPath, at: .middle, animated: true)
    ...
    func textFieldDidEndEditing(_ textField: UITextField, reason: UITextField.DidEndEditingReason) {
        ...
        if let index = customMethodManager?.getIndexPathFor(view: textField, tableView: self.otpTable) {
            if str?.count == 0 {
