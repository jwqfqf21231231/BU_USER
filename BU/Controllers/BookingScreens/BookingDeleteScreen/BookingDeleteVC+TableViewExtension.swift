//
//  BookingDeleteVC+TableViewExtension.swift
//  BU
//
...
extension BookingDeleteVC:UITableViewDataSource{
    func numberOfSections(in tableView: UITableView) -> Int {
        return (self.cancelled_Bookings == nil) ? 0 : self.cancelled_Bookings?.numberOfSections ?? 0
    }
    
    func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        return self.cancelled_Bookings?.numberOfRowsInSection(section) ?? 0
    }
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        guard let info = cancelled_Bookings?.bookingAt(indexPath.row) else {
            fatalError(ConstantTexts.noDataIndexPath)
        }
        cell.configureForCancelled(with: info)
        return cell
    }
...
extension BookingDeleteVC:UITableViewDelegate{
